...
public class Fibonacci {
    ...
    public String fibonacci(@PathVariable("number") int number) throws ConfigurationException {
        if (number < 0) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, BAD_REQUEST_MESSAGE);
        }
        Service fibService = new ServiceConfiguration().fibonacciService();

        return fibService.countNthFib(number);
    }
